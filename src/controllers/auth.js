...
module.exports = {
  login: async (req, res) => {
    ...
    if (username === 'admin') {
      ...
      if (check.length < 1) return myres(res, 404, 'authentication failed', `Username ${username} is not exist`)
      ...
      try {
        ...
        if (compare) {
          var token = jwt.sign({ sub: id, emp_number, fullname, position_id, position_name, is_head, department_id, department_name }, process.env.JWT_SECRET, { expiresIn: '6h' });
          return myres(res, 200, 'authentication success', token)
        }
        return myres(res, 401, 'authentication failed', 'Invalid Password')
      } catch (error) {
